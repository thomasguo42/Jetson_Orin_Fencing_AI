...
import debug_referee_fps30 as fps30_referee  # type: ignore
...
def _load_vpi() -> Any:
    ...
    if vpi is not None:
        return vpi
    if not VPI_PYTHON_PATH.exists():
        return None
    try:
        vpi = importlib.import_module("vpi")  # type: ignore[assignment]
    except Exception:
        vpi = None  # type: ignore[assignment]
    return vpi
...
class FisheyeFrameCorrector:
    """Apply the same fisheye correction as the legacy video path, but in memory."""

    def __init__(self, width: int, height: int, *, backend: str, strength: float = -0.18, balance: float = 0.0):
        ...
        self._opencv_map1 = None
        self._opencv_map2 = None
        self._vpi_warp = None
        self._vpi = None

        cx, cy = self.width / 2.0, self.height / 2.0
        fx = fy = max(self.width, self.height)
        camera_matrix = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]], dtype=np.float64)
        distortion = np.array([self.strength, 0, 0, 0], dtype=np.float64)
        new_camera = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(
            camera_matrix,
            distortion,
        ...
        if backend == "opencv":
            self._opencv_map1, self._opencv_map2 = analysis._build_fisheye_maps(
                self.width,
                self.height,
                strength=self.strength,
                balance=self.balance,
            )
            self.crop_x, self.crop_y, self.crop_w, self.crop_h = analysis._auto_crop_from_maps(
                self._opencv_map1,
                self._opencv_map2,
            )
            return
        ...
        if self._vpi is None:
            raise RuntimeError("VPI is not available but a VPI fisheye backend was requested.")

        grid = self._vpi.WarpGrid((self.width, self.height))
        self._vpi_warp = self._vpi.WarpMap.fisheye_correction(
            grid,
            Kin=camera_matrix[0:2, :].astype(np.float32),
            Kout=new_camera[0:2, :].astype(np.float32),
            X=np.eye(3, 4, dtype=np.float32),
            coeffs=distortion.astype(np.float32),
            mapping=self._vpi.FisheyeMapping.EQUIDISTANT,
        )
    ...
    def correct(self, frame_bgr: np.ndarray) -> np.ndarray:
        if self.backend == "opencv":
            ...
            return undistorted[self.crop_y : self.crop_y + self.crop_h, self.crop_x : self.crop_x + self.crop_w]

        assert self._vpi is not None
        backend = self._vpi.Backend.CUDA if self.backend == "vpi-cuda" else self._vpi.Backend.VIC
        with self._vpi.Backend.CUDA:
            image = self._vpi.asimage(frame_bgr).convert(self._vpi.Format.NV12_ER)
        with backend:
            corrected = image.remap(self._vpi_warp, interp=self._vpi.Interp.CATMULL_ROM)
        with self._vpi.Backend.CUDA:
            corrected_bgr = corrected.convert(self._vpi.Format.BGR8)
        output = corrected_bgr.cpu()
        if self.crop_w != self.width or self.crop_h != self.height:
            return output[self.crop_y : self.crop_y + self.crop_h, self.crop_x : self.crop_x + self.crop_w]
        return output


def _build_fisheye_frame_corrector(video_path: Path, *, backend: str) -> Optional[FisheyeFrameCorrector]:
    if backend == "none":
        return None
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        raise FileNotFoundError(f"Cannot open video for fisheye setup: {video_path}")
    width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    cap.release()
    return FisheyeFrameCorrector(width, height, backend=backend)


def _model_supports_batched_bootstrap(model: YOLO) -> bool:
    ckpt_path = str(getattr(model, "ckpt_path", "") or getattr(model, "model_name", "") or "").lower()
    return not ckpt_path.endswith(".engine")


def build_arg_parser(*, require_phrase_dir: bool = True, add_help: bool = True) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__, add_help=add_help)
    parser.add_argument(
        "--phrase-dir",
        type=Path,
        required=require_phrase_dir,
        help="Phrase folder with video/txt/keypoints.",
    )
    parser.add_argument(
        "--model-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="Ultralytics inference size for YOLO pose. Lower values reduce latency but may reduce accuracy.",
    )
    parser.add_argument(
        "--yolo-half",
        action="store_true",
        help="Enable FP16 inference for lower latency. Disabled by default to preserve baseline quality.",
    )
    parser.add_argument(
        "--yolo-verbose",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help="How many early frames to scan when selecting the front left/right fencers.",
    )
    parser.add_argument(
        "--fisheye-backend",
        choices=FISHEYE_BACKENDS,
        default="none",
        help="Backend used for in-memory fisheye correction in the low-latency path.",
    )
    parser.add_argument(
        "--frame-prefetch",
        type=int,
        default=4,
        help="How many raw frames to prefetch ahead of tracking. Set to 0 to disable threaded decode overlap.",
    )
    parser.add_argument(
        "--video-reader",
        choices=VIDEO_READERS,
        default="opencv",
        help="Frame reader backend for the tracking loop. gstreamer-mjpeg is intended for AVI/MJPEG phrase videos on Jetson.",
    )
    parser.add_argument(
        "--write-corrected-video",
        action="store_true",
        help="Write a corrected video artifact for debugging. Disabled by default in the low-latency path.",
    )
    parser.add_argument(
        "--write-yolo-overlay",
        action="store_true",
        help="Write the all-people YOLO overlay video. Disabled by default to reduce latency.",
    )
    parser.add_argument(
        "--write-repaired-overlay",
        action="store_true",
        help="Write the repaired overlay video. Disabled by default to reduce latency.",
    )
    parser.add_argument(
        "--write-bootstrap-debug",
        action="store_true",
        help="Write bootstrap initialization PNG/JSON artifacts for debugging. Disabled by default to reduce latency.",
    )
    parser.add_argument(
        "--write-repaired-excel",
        action="store_true",
        help="Write repaired keypoints XLSX. Disabled by default because the integrated referee uses in-memory data.",
    )
    parser.add_argument(
        "--write-fps30-debug-log",
        action="store_true",
        help="Write the detailed FPS30 referee debug log. Disabled by default to reduce latency.",
    )
    parser.add_argument(
        "--sam-threshold",
        type=float,
    ...
    parser.add_argument(
        "--copy-inputs",
        action="store_true",
        help="Copy phrase video/TXT into the output folder. Disabled by default; the low-latency path stages symlinks instead.",
    )
    parser.add_argument(
        "--progress",
        action="store_true",
...
def parse_args() -> argparse.Namespace:
    return build_arg_parser(require_phrase_dir=True).parse_args()
...
class JumpSafeTwoFencerTracker(analysis.TwoFencerTracker):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.kpt_cost_relax_per_miss = 0.10
        self.recovery_iou_min = 0.25
        self.recovery_iou_kpt_bonus = 0.22
    ...
    def _hard_identity_reject(
        ...
        bbox_overlap = float(analysis.bbox_iou(prev_bbox, bbox))
        ...
        kpt_threshold = self.max_kpt_cost_hard + (self.kpt_cost_relax_per_miss * miss_count)
        if miss_count > 0 and bbox_overlap >= self.recovery_iou_min:
            kpt_threshold += self.recovery_iou_kpt_bonus
        if np.isfinite(kpt_cost) and kpt_cost > kpt_threshold:
            return True
...
def extract_tracks_with_jump_safe_tracker(
    ...
    *,
    ...
    detection_half: Optional[bool] = None,
    detection_max_det: Optional[int] = None,
    prefetched_detections: Optional[Dict[int, List[np.ndarray]]] = None,
    frame_transform=None,
    frame_prefetch: int = 0,
    video_reader: str = "opencv",
    ...
    try:
        return analysis.extract_tracks_from_video(
            ...
            detection_half=detection_half,
            detection_max_det=detection_max_det,
...
def _stage_input_file(source: Path, dest: Path, *, copy_inputs: bool) -> Path:
    source = source.resolve()
    if dest.resolve() == source if dest.exists() else dest == source:
        return source
...
def _load_video_frame(video_path: Path, frame_idx: int, frame_transform=None) -> np.ndarray:
    cap = cv2.VideoCapture(str(video_path))
    if frame_idx > 0:
...
def _locate_front_fencers_with_yolo_bootstrap(
    ...
    *,
    ...
    yolo_half: bool,
    frame_transform=None,
) -> Tuple[Dict[str, object], Dict[int, List[np.ndarray]]]:
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        raise ValueError(f"Cannot open video for bootstrap: {video_path}")
    ...
    clusters: List[Dict[str, object]] = []
    bootstrap_frames_bgr: List[np.ndarray] = []
    bootstrap_frame_indices: List[int] = []
    ...
    try:
        for frame_idx in range(total_frames):
            ...
            if frame_transform is not None:
                frame = frame_transform(frame)
            scanned_frames += 1
            bootstrap_frames_bgr.append(frame)
            bootstrap_frame_indices.append(frame_idx)
    finally:
        cap.release()
    ...
    batch_size = len(bootstrap_frames_bgr) if _model_supports_batched_bootstrap(model) else None
    ...
    if batch_size is None:
        results = [model(frame, **analysis.ultralytics_predict_kwargs(
            verbose=yolo_verbose,
            conf=yolo_conf,
            imgsz=yolo_imgsz,
            half=yolo_half,
        ))[0] for frame in bootstrap_frames_bgr]
    else:
        results = model(bootstrap_frames_bgr, **predict_kwargs)

    for frame_idx, result in zip(bootstrap_frame_indices, results):
        ...
        prefetched_detections[int(frame_idx)] = detections
...
def _render_yolo_all_people_overlay(
    ...
    *,
    ...
    cap = cv2.VideoCapture(str(video_path))
    if not cap.isOpened():
        raise ValueError(f"Cannot open video for YOLO overlay: {video_path}")
...
def rerun_extraction(
    ...
    *,
    ...
    frame_prefetch: int,
    video_reader: str,
    write_corrected_video: bool,
    write_yolo_overlay: bool,
    write_bootstrap_debug: bool,
    preloaded_model: Optional[YOLO] = None,
) -> Dict[str, Any]:
    if not model_path.exists():
        raise FileNotFoundError(f"YOLO weights not found: {model_path}")

    fisheye_corrector = _build_fisheye_frame_corrector(input_video, backend=fisheye_backend)
    frame_transform = fisheye_corrector.correct if fisheye_corrector is not None else None
    corrected_video_artifact: Optional[Path] = None
    if write_corrected_video:
        corrected_video_path = output_dir / f"{input_video.stem}_corrected.mp4"
        corrected_video_artifact = Path(
            correct_fisheye_video(
    ...
    yolo_all_people_overlay: Optional[Path] = None
    if write_yolo_overlay:
    ...
    bootstrap_prefetched_detections: Optional[Dict[int, List[np.ndarray]]] = None
    ...
    if tracking_indices is not None:
        ...
        bootstrap_loc, bootstrap_prefetched_detections = _locate_front_fencers_with_yolo_bootstrap(
            input_video,
            model,
        ...
        if bootstrap_loc["left"] is not None and bootstrap_loc["right"] is not None:
            ...
            if write_bootstrap_debug:
                init_frame_rgb = _load_video_frame(input_video, initial_frame_index, frame_transform=frame_transform)
            ...
            tracks_per_frame = extract_tracks_with_jump_safe_tracker(
                ...
                detection_imgsz=yolo_imgsz,
                detection_half=yolo_half,
                prefetched_detections=bootstrap_prefetched_detections,
                frame_transform=frame_transform,
                frame_prefetch=frame_prefetch,
                video_reader=video_reader,
            )
        else:
            if write_bootstrap_debug and bootstrap_loc is not None:
                fallback_frame_rgb = _load_video_frame(input_video, 0, frame_transform=frame_transform)
    ...
    return {
        ...
        "corrected_video_artifact": str(corrected_video_artifact) if corrected_video_artifact is not None else None,
        "frame_transform": frame_transform,
        ...
        "yolo_all_people_overlay": str(yolo_all_people_overlay) if yolo_all_people_overlay is not None else None,
...
def run_phrase_job(
    ...
    *,
    ...
    staged_video = _stage_input_file(input_video, output_dir / input_video.name, copy_inputs=args.copy_inputs)
    staged_txt = _stage_input_file(txt_path, output_dir / txt_path.name, copy_inputs=args.copy_inputs)
    ...
    if args.repair_only:
        ...
        if source_corrected is None:
            working_video_path = staged_video
        else:
            working_video_path = _stage_input_file(
                source_corrected,
                output_dir / source_corrected.name,
        ...
        extraction = {
            ...
            "corrected_video_artifact": str(working_video_path),
            "frame_transform": None,
            "fisheye_backend": None,
            "tracks_per_frame": [],
            "tracking_indices": _read_tracking_indices(staged_txt),
            "manual_indices_present": _read_tracking_indices(staged_txt) is not None,
        ...
        extraction = rerun_extraction(
            ...
            write_corrected_video=args.write_corrected_video,
            write_yolo_overlay=args.write_yolo_overlay,
            write_bootstrap_debug=args.write_bootstrap_debug,
            preloaded_model=preloaded_model,
        )
        working_video_path = extraction["working_video"]
    ...
    repaired_excel: Optional[Path] = None
    if args.write_repaired_excel or args.repair_only:
    ...
    repaired_overlay: Optional[Path] = None
    if args.write_repaired_overlay:
        ...
        render_overlay_video(
            ...
            frame_transform=extraction.get("frame_transform"),
        )

    phrase = parse_txt_file(str(staged_txt), video_path=str(working_video_path))
    ...
    fps30_debug_log: Optional[Path] = None
    if args.write_fps30_debug_log:
        fps30_debug_log = output_dir / "debug_referee_fps30.txt"
    final_fps30_decision = fps30_referee.run_referee_on_keypoints(
        ...
        debug_output_path=fps30_debug_log,
        debug_logging=args.write_fps30_debug_log,
    )
    ...
    result = {
        ...
        "fisheye_backend": extraction.get("fisheye_backend"),
        "yolo_imgsz": extraction.get("yolo_imgsz"),
        "yolo_half": extraction.get("yolo_half"),
        "corrected_video_artifact": extraction.get("corrected_video_artifact"),
        ...
        "repaired_excel": str(repaired_excel) if repaired_excel is not None else None,
        "repaired_overlay": str(repaired_overlay) if repaired_overlay is not None else None,
        "normalisation_constant": norm_constant,
        "video_angle": video_angle,
        ...
        "fps30_debug_log": str(fps30_debug_log) if fps30_debug_log is not None else None,
    }
...
def main() -> int:
    args = parse_args()
    job_result = run_phrase_job(args)
    print(json.dumps(job_result["summary"], indent=2))
    return 0
