...
GI_PYTHON_PATH = Path("/usr/lib/python3/dist-packages")
if GI_PYTHON_PATH.exists():
    site.addsitedir(str(GI_PYTHON_PATH))
...
def _iter_video_frames(
    ...
    *,
    ...
    frame_queue: "queue.Queue[object]" = queue.Queue(maxsize=max(1, int(frame_prefetch)))
    sentinel = object()
    ...
    def _producer() -> None:
        ...
        try:
            while True:
                ...
                frame_idx += 1
        except BaseException as exc:  # pragma: no cover - worker thread propagation
            error_holder["exc"] = exc
        finally:
            cap.release()
            frame_queue.put(sentinel)

    producer = threading.Thread(target=_producer, name="frame-prefetch", daemon=True)
    producer.start()
    try:
        while True:
            ...
            if item is sentinel:
                break
            frame_idx, frame = item  # type: ignore[misc]
            if frame_transform is not None:
                frame = frame_transform(frame)
            yield frame_idx, frame
        if "exc" in error_holder:
            raise RuntimeError("Frame prefetch failed") from error_holder["exc"]
    finally:
        producer.join()


def _iter_gstreamer_mjpeg_frames(
    video_path: str,
    *,
    frame_transform: Optional[Callable[[np.ndarray], np.ndarray]] = None,
    max_buffers: int = 4,
    ...
    if Gst is None:
        raise RuntimeError("GStreamer Python bindings are not available for gstreamer-mjpeg video reading.")

    buffer_count = max(1, int(max_buffers))
    pipeline_desc = (
        f'filesrc location="{video_path}" ! avidemux ! jpegparse ! '
        'nvv4l2decoder mjpeg=true enable-max-performance=true ! '
        'nvvidconv ! video/x-raw,format=BGRx ! videoconvert ! video/x-raw,format=BGR ! '
        f'appsink name=sink sync=false max-buffers={buffer_count} drop=false emit-signals=false'
    )
    pipeline = Gst.parse_launch(pipeline_desc)
    sink = pipeline.get_by_name("sink")
    if sink is None:
        pipeline.set_state(Gst.State.NULL)
        raise RuntimeError("Failed to create GStreamer appsink for MJPEG video reading.")
        ...
        while True:
            ...
            if sample is None:
                ...
                if message is not None and message.type == Gst.MessageType.ERROR:
                    ...
                    raise RuntimeError(f"GStreamer MJPEG decode failed for {video_path}: {err} ({debug})")
                break
            ...
            if not ok:
                raise RuntimeError(f"Failed to map GStreamer buffer for {video_path}")
            try:
...
def _optional_pose_descriptor(kpts: Optional[np.ndarray]) -> Optional[Dict[str, Dict[str, float]]]:
    if kpts is None or len(kpts) < 17:
        return None
    return _pose_descriptor(kpts)
...
def _descriptor_distance(
    det_desc: Optional[Dict[str, Dict[str, float]]],
    ref_desc: Optional[Dict[str, Dict[str, float]]],
) -> float:
    if ref_desc is None or det_desc is None:
        return float("inf")
...
def temporal_posture_distance_from_descriptors(
    det_desc: Optional[Dict[str, Dict[str, float]]],
    ref_desc: Optional[Dict[str, Dict[str, float]]],
) -> Optional[float]:
    d = _descriptor_distance(det_desc, ref_desc)
    if not np.isfinite(d):
        return None
    return float(d)


def composite_track_cost_with_meta(
    k_det: np.ndarray,
    det_bbox: Optional[Tuple[int, int, int, int]],
    det_center: Optional[Tuple[float, float]],
    det_descriptor: Optional[Dict[str, Dict[str, float]]],
    k_prev: np.ndarray,
    prev_bbox: Optional[Tuple[int, int, int, int]],
    prev_descriptor: Optional[Dict[str, Dict[str, float]]],
    prev_torso_scale: float,
    pred_center: Optional[Tuple[float, float]],
    frame_diag: float,
):
    kpt_dist = robust_kpt_distance(k_det, k_prev)
    if det_bbox is None or prev_bbox is None:
        ...
        iou = bbox_iou(det_bbox, prev_bbox)
        bbox_cost = 1.0 - iou
    if pred_center is not None and det_center is not None:
        motion_dist = math.hypot(det_center[0] - pred_center[0], det_center[1] - pred_center[1])
        motion_cost = motion_dist / frame_diag if frame_diag > 0 else 0.0
    else:
        motion_cost = 0.0
    scale = prev_torso_scale if prev_torso_scale > 0 else 1.0
    kpt_cost = kpt_dist / scale if scale > 0 else kpt_dist
    posture_cost = temporal_posture_distance_from_descriptors(det_descriptor, prev_descriptor)
    if posture_cost is None:
        return 0.7 * kpt_cost + 0.2 * bbox_cost + 0.1 * motion_cost
    return 0.45 * kpt_cost + 0.35 * posture_cost + 0.12 * bbox_cost + 0.08 * motion_cost
...
def posture_distance_to_reference(det_kpts: np.ndarray, ref_kpts: Optional[np.ndarray]) -> float:
    ...
    det_desc = _optional_pose_descriptor(det_kpts)
    ref_desc = _optional_pose_descriptor(ref_kpts)
    return _descriptor_distance(det_desc, ref_desc)
...
class _TrackState:
    def __init__(
        ...
        self.torso_scale = torso_scale(kpts)
        self.descriptor = _optional_pose_descriptor(kpts)
        ...
        self.anchor_area = float(bbox_area(self.anchor_bbox)) if self.anchor_bbox is not None else 0.0
        self.anchor_descriptor = _optional_pose_descriptor(self.anchor_kpts)
...
class TwoFencerTracker:
    ...
    def build_detection_meta(self, det_kpts: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "descriptor": _optional_pose_descriptor(det_kpts),
        }

    def build_detection_meta_list(self, detections: List[np.ndarray]) -> List[Dict[str, Any]]:
        return [self.build_detection_meta(det_kpts) for det_kpts in detections]
    ...
    def _soft_match_cost(
        ...
        base = composite_track_cost_with_meta(
            det_kpts,
            bbox,
            center,
            meta.get("descriptor"),
        ...
        if track.anchor_kpts is not None:
            anchor_posture = temporal_posture_distance_from_descriptors(
                meta.get("descriptor"),
                track.anchor_descriptor,
            )
        ...
        if track.anchor_bbox is not None:
            anchor_area = max(float(track.anchor_area), 1.0)
    ...
    def update(
        self,
        detections: List[np.ndarray],
        det_meta: Optional[List[Dict[str, Any]]] = None,
        ...
        if len(detections) == 0:
            ...
            return
        if det_meta is None:
            det_meta = self.build_detection_meta_list(detections)
        if costs is None:
            costs = self.build_costs(detections, det_meta)
    ...
    def _overwrite_track(self, track: _TrackState, kpts: np.ndarray):
        ...
        track.torso_scale = torso_scale(kpts)
        track.descriptor = _optional_pose_descriptor(kpts)
...
def ultralytics_predict_kwargs(
    *,
    verbose: bool,
...
def extract_tracks_from_video(
    ...
    elif video_reader == "gstreamer-mjpeg":
        probe_cap = cv2.VideoCapture(video_path)
        if not probe_cap.isOpened():
            raise ValueError(f"Cannot open video metadata probe: {video_path}")
        width = int(probe_cap.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(probe_cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
        probe_cap.release()
        frame_iter = _iter_gstreamer_mjpeg_frames(
            video_path,
            frame_transform=frame_transform,
            max_buffers=frame_prefetch,
        )
    else:
        raise ValueError(f"Unsupported video_reader '{video_reader}'")

    tracker = TwoFencerTracker(frame_w=width, frame_h=height)
    tracks_per_frame = []
    predict_kwargs = ultralytics_predict_kwargs(
    ...
    def _augment_detections_with_track_crops(
        ...
        for tid in (0, 1):
            ...
            for d_idx, det_kpts in enumerate(augmented):
            ...
            for det_kpts in crop_kpts:
                ...
                global_kpts[valid, 0] += ix1
                global_kpts[valid, 1] += iy1
                meta = tracker.build_detection_meta(global_kpts)
            ...
            best_bbox = best_det_meta["bbox"] if best_det_meta is not None else bbox_from_keypoints(best_det)
            ...
            if not duplicate:
                ...
                augmented_meta.append(best_det_meta if best_det_meta is not None else tracker.build_detection_meta(best_det))
                new_idx = len(augmented) - 1
                for active_tid in (0, 1):
                    active_track = tracker.tracks.get(active_tid)
                    if active_track is None:
                        continue
                    cost = tracker._soft_match_cost(active_tid, active_track, best_det, augmented_meta[new_idx])
                    if cost is not None:
                        augmented_costs[(active_tid, new_idx)] = cost

        return augmented, augmented_meta, augmented_costs, recovered_by_tid

    def _snapshot_frame_tracks(active_tracker: TwoFencerTracker, observed_frame_idx: int) -> List[Dict[str, Any]]:
        frame_tracks = []
        for tid in (0, 1):
            track = active_tracker.tracks.get(tid)
            observed = track is not None and track.last_seen == observed_frame_idx
            kpts = active_tracker.get_track(tid) if observed else None
    ...
    def _backfill_preinit_tracks() -> None:
        if initial_frame_index <= 0 or not prefetched_detections:
            return
        if initial_frame_index >= len(tracks_per_frame):
            return

        seed_tracks = tracks_per_frame[initial_frame_index]
        ...
        backfill_tracker = tracker.__class__(width, height, max_miss=tracker.max_miss)
        initialized = backfill_tracker.initialize_with_keypoints(
            np.array(left_seed, copy=True),
            np.array(right_seed, copy=True),
        )
        if not initialized:
            return

        for backfill_frame_idx in range(initial_frame_index - 1, -1, -1):
            detections = [
                np.array(det_kpts, copy=True)
                for det_kpts in prefetched_detections.get(backfill_frame_idx, [])
            ]
            ...
            tracks_per_frame[backfill_frame_idx] = _snapshot_frame_tracks(backfill_tracker, observed_frame_idx)

    for frame_idx, frame in frame_iter:
        height, width = frame.shape[:2]
        detections = []
        if prefetched_detections is not None and frame_idx in prefetched_detections:
            detections = [np.array(det, copy=True) for det in prefetched_detections[frame_idx]]
        else:
        ...
        det_meta = tracker.build_detection_meta_list(detections)
        costs: Dict[Tuple[int, int], float] = {}
        if tracker.initialized and detections:
            costs = tracker.build_costs(detections, det_meta)

        recovered_by_tid: Dict[int, np.ndarray] = {}
        if frame_idx > 0 and tracker.initialized:
            detections, det_meta, costs, recovered_by_tid = _augment_detections_with_track_crops(
                frame,
                detections,
                det_meta,
                costs,
        ...
        if forced_init_applied:
            ...
            tracker.update(detections, det_meta=det_meta, costs=costs)
        ...
        current_frame_idx = tracker.frame_idx - 1
        tracks_per_frame.append(_snapshot_frame_tracks(tracker, current_frame_idx))

    _backfill_preinit_tracks()
