#!/usr/bin/env python3
"""
Run the jump-safe limb-interp extraction/repair pipeline over many phrase folders.

This is the Raspberry Pi bundle batch runner. It defaults to the bundle-local
runtime input/output folders but accepts any input root.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base-dir",
        type=Path,
        default=BUNDLE_ROOT / "runtime_inputs",
        help="Directory containing phrase folders.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=BUNDLE_ROOT / "runtime_outputs" / "experimental_limb_interp_jumpsafe",
        help="Per-phrase output root passed to the experimental script.",
    )
    parser.add_argument(
        "--log-dir",
        type=Path,
        default=BUNDLE_ROOT / "logs" / "experimental_limb_interp_jumpsafe_batch",
...
def main() -> int:
    ...
    script_path = (Path(__file__).resolve().parent / "reprocess_phrase_limb_interp_jumpsafe_experimental.py").resolve()
    ...
    with log_path.open("a", encoding="utf-8") as log:
        for index, folder in enumerate(folders, 1):
            ...
            print(
                json.dumps({"status": rec["status"], "name": folder.name, "seconds": rec["seconds"]}, indent=2),
                flush=True,
            )
