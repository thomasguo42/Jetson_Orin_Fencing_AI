...
FISHEYE_BACKENDS = ("vpi-cuda", "vpi-vic", "opencv")
...
class FisheyeFrameCorrector:
    ...
    def __init__(self, width: int, height: int, *, backend: str, strength: float = -0.18, balance: float = 0.0):
        ...
        grid = vpi.WarpGrid((self.width, self.height))
        self._vpi_warp = vpi.WarpMap.fisheye_correction(
            grid,
    ...
    def correct(self, frame_bgr: np.ndarray) -> np.ndarray:
        ...
        backend = vpi.Backend.CUDA if self.backend == "vpi-cuda" else vpi.Backend.VIC
        with vpi.Backend.CUDA:
            image = vpi.asimage(frame_bgr).convert(vpi.Format.NV12_ER)
        with backend:
            corrected = image.remap(self._vpi_warp, interp=vpi.Interp.CATMULL_ROM)
        with vpi.Backend.CUDA:
            corrected_bgr = corrected.convert(vpi.Format.BGR8)
...
def _build_fisheye_frame_corrector(video_path: Path, *, backend: str) -> FisheyeFrameCorrector:
...
def rerun_extraction(
    ...
    *,
    ...
    frame_transform = fisheye_corrector.correct
